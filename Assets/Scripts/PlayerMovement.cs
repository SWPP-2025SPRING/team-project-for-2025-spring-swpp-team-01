...
public class PlayerMovement : MonoBehaviour
{
    ...
    void TryMount()
    {
        ...
        foreach (var hit in hits)
        {
            if (hit.CompareTag("Bug"))
            {
                var ladybug = hit.GetComponentInParent<LadybugMovement>();
                if (ladybug != null)
                {
                    Mount(ladybug.transform);
                    break;
                }
            }
        }
    ...
    public void Unmount()
    {
        ...
        Vector3 dismountDir = -transform.forward;  // 시선 반대 방향
        Vector3 dismountPos = mountedBug.position + dismountDir.normalized * 1.5f + Vector3.down * 0.3f;

        transform.SetParent(null);
        transform.position = dismountPos;
        transform.rotation = Quaternion.Euler(0f, mountedBug.eulerAngles.y, 0f);
    ...
    public void ForceFallFromBug()
    {
        ...
        controller.enabled = true; // 여기서 미리 켜줌
        Fall(); // 낙하 모션 및 물리 적용
    }
